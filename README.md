...
- **Sketches.** Twenty hand-drawn SVGs live in `public/sketches/` (door, key, lantern, lighthouse, ship, moon, stranger, scroll, tree, planet, sword, crown, candle, goblet, bird, mountain, bridge, eye, compass, hourglass), described in `src/lib/sketches.ts`. Each shows one subject, with nothing else in the frame. When writing a page, the model also picks the sketch that fits best (or none), avoiding the previous page's. They are black lines on white, and each theme recolours them in CSS. To add one, draw a 200×200 SVG in the same style, save it in `public/sketches/`, and add its id and a description to `src/lib/sketches.ts`.
